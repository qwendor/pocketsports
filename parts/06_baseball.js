...
SPORTS.baseball={
  ...
  contact(q,hv,b){ const K=BASE_CFG; const sp=hv.length(); const m=this.miis[this.turn];
    ...
    b.vel.set(Math.sin(th)*Math.cos(ph)*v,Math.sin(ph)*v,-Math.cos(th)*Math.cos(ph)*v); b.phase='hit'; b.landed=false; b.pos.z=Math.min(b.pos.z,.2); this.state='hit'; this.stateT=0; AUD.crack(); const p=this.cur(); if(!p.cpu)buzz(p,120); },
